...
#include <string.h>
#include <stdlib.h>
//To check the password for deductions
char* check(char* str){
	...
	if(*p!=10){
		//Make the difference variable the 10 minus the actual length of the password
		*d=10-len;
		//multiply the difference by 5 because that is the amount per deduction
		count=5**d;
		
		//if the count for the deductions is greater than 30 print it and statement
		if(count>30){
...
int main(){
	...
	scanf("%s",password);
	//run check function for password
	check(password);
	//return nothing
	return 0;
	}
